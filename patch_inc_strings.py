...
inc_dir = r"C:\Users\rodd\Desktop\BWifiKill-ESP32-V4.0-main\include"
for f in os.listdir(inc_dir):
    if f.endswith('.cpp') or f.endswith('.h'):
        filepath = os.path.join(inc_dir, f)
        with open(filepath, 'r', encoding='utf-8', errors='ignore') as file:
            content = file.read()
        
        # Replace Spanish strings and PepeAngell
        content = content.replace("PepeAngell", "MX 100RAV")
        content = content.replace("pepeangell", "mxsourav")
...
print("Firmware INC strings patched.")
